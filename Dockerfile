...
#
# declmig-web-server — production container image.
#
...
#   docker buildx build --platform linux/arm64 -t ghcr.io/declarative-migrations/declmig-web-server:dev --load .
#
...
#     -t ghcr.io/declarative-migrations/declmig-web-server:dev --load .
#
#   # OCI image index with provenance and SBOM attestations:
#   docker buildx build --platform linux/arm64 --push \
#     --provenance=true --sbom=true \
#     --output type=image,oci-mediatypes=true,name=ghcr.io/declarative-migrations/declmig-web-server:dev .
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    cargo build --release --bin declmig-web-server; \
    strip target/release/declmig-web-server; \
    cp target/release/declmig-web-server /usr/local/bin/declmig-web-server
...
COPY --from=builder --chown=65532:65532 /usr/local/bin/declmig-web-server /usr/local/bin/declmig-web-server
ENV DECLMIG_WEB_BIND=0.0.0.0:8081
EXPOSE 8081
USER 65532:65532
ENTRYPOINT ["/usr/local/bin/declmig-web-server"]
...
COPY --from=builder /usr/local/bin/declmig-web-server /usr/local/bin/declmig-web-server
...
#     ghcr.io/declarative-migrations/declmig-web-server:dev
#
...
CMD ["/usr/local/bin/declmig-web-server"]
