# The four avenues between `declmig-web-server` and `declmig-api-server`

`declarative-migrations` follows the fleet-wide contract in
...
|---|------|------|-------------------|
| 1 | `direct_read` | `declmig-web-server` → Postgres, via `declmig-lib-core` | a read on the page's critical path |
| 2 | `http` | `declmig-web-server` → `declmig-api-server`, stateless | the default, and anything that writes |
| 3 | `tcp` | `declmig-web-server` ⇄ `declmig-api-server`, held open | chatty internal calls where the handshake dominates |
| 4 | `jet_stream` | `declmig-web-server` → NATS → `declmig-api-server` | work the caller does not need to watch finish |

All four carry the same envelope and land on the same handler, so a
`declmig-e2e` suite can drive one operation four ways and assert the answers
...
|---|---|
| Slug | `declmig` |
| Environment prefix | `DECLMIG_` |
| Request subject | `declmig.api.operation.v1` |
| Result subject | `declmig.api.operation-result.v1` |
| Durable consumer | `declmig-api-server-v1` |
...
|----------|--------|--------------|
| `DECLMIG_READONLY_DATABASE_URL` | 1 | no direct reads |
| `DECLMIG_API_URL` | 2 | no stateless HTTP |
| `DECLMIG_API_MTLS_ADDR` | 3 | no stateful TCP |
| `DECLMIG_API_TLS_SERVER_NAME` | 3 | plaintext to a TLS-terminating mesh |
| `DECLMIG_WEB_CLIENT_CERT_FILE` | 3 | ” |
| `DECLMIG_WEB_CLIENT_KEY_FILE` | 3 | ” |
| `DECLMIG_API_CA_FILE` | 3 | ” |
| `DECLMIG_NATS_URL` | 4 | no asynchronous avenue |

A missing variable disables its avenue. A variable that is present and unsafe
fails startup: a plaintext `DECLMIG_API_URL` off loopback, a
`DECLMIG_NATS_URL` that is not `tls://`, or mutual TLS with two of its
four files set.

In-cluster, `DECLMIG_NATS_URL` points at the messaging service defined in
...
**In `declmig-web-server`** — implement `DirectReader` over `declmig-lib-core`:
...
impl DirectReader<Operation, Projection> for LibCoreReader {
        ...
        -> Result<Projection, TransportError>
    {
        ...
        // not an access check.
        // Then build the query with declmig-lib-core's query builders.
    }
...
**In `declmig-api-server`** — implement `OperationHandler` once, and hand the same
...
**Avenue 1 is read-only twice over.** `declmig-lib-core` gates its write surface
behind a `read-write` cargo feature and migrations behind `migrate`. Depend on
it from `declmig-web-server` as:

```toml
declmig-lib-core = { git = "…", default-features = false, features = ["read-only"] }
```

so a mutating call is a *compile error*, not a code-review miss. Separately,
`DECLMIG_READONLY_DATABASE_URL` must name a Postgres role without
`INSERT`, `UPDATE`, `DELETE` or DDL. A mistake has to defeat both. Migrations
belong to `declmig-lib-core` and `declarative-migrations`, never to a web server.
