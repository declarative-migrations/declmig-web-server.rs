...
#
# Multi-stage image for declmig-web-server.
# Prefer linux/arm64:
#   docker buildx build --platform linux/arm64 -t declmig-web-server:dev .
#   docker run --rm --platform linux/arm64 \
#     -e SOPS_AGE_KEY="$(cat ~/.config/sops/age/keys.txt)" declmig-web-server:dev
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    --mount=type=cache,target=/src/target,id=declmig-web-server-target-${TARGETARCH},sharing=locked \
...
COPY --from=build "/usr/local/bin/declmig-web-server" "/usr/local/bin/declmig-web-server"
...
RUN --mount=type=bind,source=.,target=/src,ro \
    ...
    && chown -R 65532:65532 /app /usr/local/bin/declmig-web-server
ENV SOPS_SECRETS_FILE=/app/secrets/app.env \
    OTEL_SERVICE_NAME=declmig-web-server \
    OTEL_EXPORTER_OTLP_ENDPOINT=http://dd-otel-collector.observability.svc.cluster.local:4318 \
    RUST_LOG=info
USER 65532:65532
EXPOSE 8080
ENTRYPOINT ["/usr/local/bin/sops-entrypoint.sh", "/usr/local/bin/declmig-web-server"]
