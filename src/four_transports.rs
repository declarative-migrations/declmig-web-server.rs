#![forbid(unsafe_code)]
//! The four avenues this web server can use to reach `declmig-api-server`.
//!
...
//! to declarative-migrations — the environment prefix, the service slug that names its NATS
//! subjects, and the api route that accepts an envelope.
//!
//! | # | Mode | What happens |
//! |---|------|--------------|
//! | 1 | `direct_read` | this server queries Postgres itself through `declmig-lib-core`, read-only |
//! | 2 | `http` | stateless request to the `declmig-api-server` cluster |
//! | 3 | `tcp` | framed, held-open connection to the `declmig-api-server` cluster |
...
//! `declmig-lib-core`'s read context, and hand it to [`gateway_from_env`]. See
...
///
/// `DECLMIG_API_URL`, `DECLMIG_NATS_URL`, and the rest; see the
